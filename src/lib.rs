...
impl GodwokenHost {
    fn new() -> Self {
        let rollup_config = RollupConfig::new_builder()
            ...
            .build();
        let state = DummyState::default(); // will be reset
    ...
    fn get_raw(&mut self, key: &H256) -> Result<H256> {
        let value = match self.run_result.write.write_values.get(key) {
            Some(value) => *value,
            None => {
                let tree_value = self
                    .state
                ...
                self.run_result.read_values.insert(*key, tree_value);
                tree_value
            }
        };
        Ok(value)
    ...
    fn get_account_count(&self) -> Result<u32> {
        if let Some(id) = self.run_result.write.account_count {
            Ok(id)
        } else {
            let count = self
                .state
    ...
    fn get_script(&mut self, script_hash: &H256) -> Option<Script> {
        let opt_script = self
            ...
            .get(script_hash)
            .cloned()
            .or_else(|| self.state.get_script(script_hash));
...
pub extern "C" fn gw_create(
    ...
    let mut result = &mut HOST.lock().unwrap().run_result;
    result.write.write_values.insert(
    ...
    // insert script
    result.write.new_scripts.insert(script_hash.into(), script);
    result.write.account_count = Some(id + 1);
...
pub extern "C" fn gw_store_data(data_addr: *const u8, len: u64) -> c_int {
    ...
    result.write.write_values.insert(data_hash_key, H256::one());
    // write data
    result.write.write_data.insert(data_hash.into(), data);
    SUCCESS
}
...
pub extern "C" fn gw_load_data(
    ...
    host.run_result.read_data.insert(data_hash, data);
    SUCCESS
}
...
pub extern "C" fn gw_log(account_id: u32, service_flag: u8, len: u64, data: *const u8) -> c_int {
    ...
    let log_item = LogItem::new_builder()
        ...
        .build();
    HOST.lock().unwrap().run_result.write.logs.push(log_item);
    SUCCESS
}
